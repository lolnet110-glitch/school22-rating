...
def update_class(class_id: int, item: ClassUpdate):
    ...
    cursor.execute(
        f"UPDATE classes SET {', '.join(fields)} WHERE id = %s",
        values
    )
...
def delete_class(class_id: int):
    ...
    cursor.execute(
        "UPDATE classes SET status = 'archive' WHERE id = %s",
        (class_id,)
    )
...
def delete_category(category_id: int):
    ...
    cursor.execute(
        "UPDATE categories SET status = 'archive' WHERE id = %s",
        (category_id,)
    )

    cursor.execute(
        "UPDATE subcategories SET status = 'archive' WHERE category_id = %s",
        (category_id,)
    )
...
def update_subcategory(subcategory_id: int, subcategory: SubcategoryUpdate):
    ...
    cursor.execute(
        f"UPDATE subcategories SET {', '.join(fields)} WHERE id = %s",
        values
    )
...
def delete_subcategory(subcategory_id: int):
    ...
    cursor.execute(
        "UPDATE subcategories SET status = 'archive' WHERE id = %s",
        (subcategory_id,)
    )
...
def create_uniform_check(class_id: int, item: UniformCheckCreate):
    ...
    return {
        "id": check_id,
        "message": "Проверка формы добавлена"
    }
...
def update_uniform_check(check_id: int, item: UniformCheckUpdate):
    ...
    cursor.execute(
        f"UPDATE uniform_checks SET {', '.join(fields)} WHERE id = %s",
        values
    )
...
def build_class_category_scores(connection, class_id: int):
    ...
    for category in categories:
        ...
        for subcategory in subcategories:
            ...
            sub_item["points"] = score["points"] if score else 0
            sub_item["comment"] = score["comment"] if score else None
            sub_item["updated_at"] = str(score["updated_at"]) if score and score["updated_at"] else None

            sub_sum += sub_item["points"]
